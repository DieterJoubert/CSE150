    ...
    if is_complete(get_board):
      ...
      for (name, (y_delta,x_delta)) in direction:
        ...
        if (0 <= y_zero + y_delta <= y_max) and (0 <= x_zero + x_delta <= x_max):
          ...
          newtup = (f_value, priority_second, list(new_path), new_board, new_depth) 
          
          if hash_fn(new_board) not in explored and newtup not in tree:
